...
> **Delivered in two phases.** Steps 1–3 — account, repository, collaborator access — are what
> Julie does now, and she has a standalone Word version of exactly those:
> `Julie_Tersigni_GitHub_Setup.docx`, generated by `/tmp/li/make_guides.py <out> setup`.
> Step 4 onward is held back until her résumé and the last two case studies land, and ships as
> a second short guide (`… publish`). This file stays the complete reference for both.
...
## Step 3 — Give Joon access, so he can upload
...
`<your-username>.github.io`. Check under **Settings → General → Repository name**.
